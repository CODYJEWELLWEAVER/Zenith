...
class SilentModeIndicator(Box):
    def __init__(self, **kwargs):
        super().__init__(
            name="silent-mode-indicator",
            style_classes="sys-info-box",
            children=[
                Label(
                    markup=Icons.mood_silence,
                    h_align="center",
                    h_expand=True,
        ...
        notification_service.connect("silent-mode-changed", self.on_silent_mode_changed)

    def on_silent_mode_changed(self, service, is_silenced: bool):
        if is_silenced:
            self.show()
        else:
            self.hide()
